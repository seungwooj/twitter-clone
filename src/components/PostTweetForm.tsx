...
export default function PostTweetForm() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // check whether the tweet is valid or not (not empty, not too long)
    if (!user || loading || tweet === "" || tweet.length > 180) return;
    try {
      setLoading(true);
      // add tweet to firestore Database
      ...
      if (file) {
        const photoRef = ref(storage, `tweets/${user.uid}/${doc.id}`);
        const result = await uploadBytes(photoRef, file);
        const url = await getDownloadURL(result.ref);
        // update tweet with the file url
